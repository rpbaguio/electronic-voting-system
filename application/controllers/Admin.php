...
class Admin extends CI_Controller {
	...
	public function update_person_info() {
		...
		$this->form_validation->set_rules('first_name', 'First Name', 'trim|required|xss_clean')->set_rules('last_name', 'Last Name', 'trim|required|xss_clean')->set_rules('access_code', 'Access Code', 'trim|required|xss_clean')->set_error_delimiters('<li>', '</li>');
	...
	public function add_person() {
		...
		$this->form_validation->set_rules('first_name', 'First Name', 'trim|required|xss_clean')->set_rules('last_name', 'Last Name', 'trim|required|xss_clean')->set_rules('gender', 'Gender', 'trim|required|xss_clean')->set_error_delimiters('<li>', '</li>');
	...
	public function person_data() {
		if(logged_in() && $this->role == 1) {
			$this->datatables->select('tbl_person.id AS person_id, tbl_person.is_validated, tbl_person.is_voted, tbl_person.is_candidate, tbl_person_info.prefix, tbl_person_info.suffix, tbl_person_info.first_name, tbl_person_info.last_name, tbl_person_info.gender', false)->from('tbl_person')->join('tbl_person_info', 'tbl_person_info.id = tbl_person.id', 'left')->where('tbl_person.role_id = 2')->where('tbl_person.is_deleted = 0');
